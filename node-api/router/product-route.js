...
router.post('/api/saveproduct',(req, res)=>{
  ...
  let productDataObject = new ProductDataModel(req.body); 
  //this creates mongoose model to be used as to make queries

  productDataObject.save()
  .then((newProductData)=>{       
    res.send(newProductData); 
    //if product successfully saved we will get the mongodb unique _id
  })
  .catch((err)=>{
    console.log("err ", err)
